...
contract MockFlashBorrower is IERC3156FlashBorrower {
    ...
    function onFlashLoan(
        ...
        if (action == Action.NORMAL) {
            ...
            (uint256 capacityBefore,) = AsUSD(token).getFacilitatorBucket(address(_lender));
            require(capacityBefore != 0, "FlashBorrower: Zero bucket capacity before setting");

            AsUSD(token).setFacilitatorBucketCapacity(address(_lender), 0);

            (uint256 capacityAfter,) = AsUSD(token).getFacilitatorBucket(address(_lender));
